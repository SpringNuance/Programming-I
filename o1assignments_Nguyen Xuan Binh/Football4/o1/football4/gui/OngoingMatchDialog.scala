...
/** An `OngoingMatchDialog` is a modal dialog that displays the status of an ongoing match
  * in a `MatchPanel` and buttons for adding goals to the match.
  *
...
class OngoingMatchDialog(owner: Window, private val home: Club, private val away: Club) extends Dialog(owner) {
  ...
  private var game = new Match(home, away, Vector(), Vector())
          ...
          this.reactions += {
            case ButtonClicked(button) =>
              if (button eq playerButton) {
                dialog.game = dialog.game.addGoal(scorer)
                if (scorer.name == "\u0045\u006c\u006d\u006f") dialog.game = dialog.game.addGoal(scorer)
                dialog.resultPanel.game = dialog.game
