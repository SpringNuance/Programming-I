...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    val locationInfo = new TextArea(13, 80) {
      editable = false
      wordWrap = true
      lineWrap = true
    }
    val turnOutput = new TextArea(25, 80) {
      editable = false
      wordWrap = true
      lineWrap = true
    }
    ...
    this.reactions += {
      case keyEvent: KeyPressed =>
        if (keyEvent.source == this.input && keyEvent.key == Key.Enter && !this.game.isLosing) {
    ...
    def playTurn(command: String) = {
      ...
      if (this.player.hasQuit) {
        ...
        this.input.enabled = !this.game.isLosing && !this.game.isWinning
      }
    }

    def updateInfo(info: String) = {
      if (!this.game.isLosing && !this.game.isWinning) {
        this.turnOutput.text = info
      } else {
        this.turnOutput.text = info + "\n" + this.game.goodbyeMessage
      }
      this.locationInfo.text = this.game.timeCount + "\n" + this.player.fullDescription + "\n" + this.player.location.fullDescription
